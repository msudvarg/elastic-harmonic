...
void HK_Reader::read_csv2() {
    std::cout << "start reading HK..." << std::endl;
    std::ifstream file(filename_);
    std::string line;
    ...
    // std::ofstream read_time("../../result_/run_time/hk_reader.txt");
    paramt_FIMS& fims = paramt_FIMS::getInstance();
    int loop = 0;
    int duration_ = fims.HK_reading_duration / 500;
    while (true) {
        // std::unique_lock<std::mutex> lock(data_mutex_);
        // data_cv_.wait_for(lock, std::chrono::milliseconds(100), [this] { return stop_thread_; });
        ...
        // std::cout << "latest hk time: " << hk_deque_.back().UTC << std::endl;
        ...
        //     std::this_thread::sleep_for(std::chrono::milliseconds(500 - duration.count()));
        // } 
    }
}
