...
time = []
for i in range(len(data_inversion_duration)):
    n_fixed_array_matlab_ = sum_every_n_rows(n_Dp_fixed_matlab, i+1)
    n_fixed_array_matlab.append(n_fixed_array_matlab_)
    
n_fixed_array_cpp = []
for i in range(len(data_inversion_duration)):
    n_Dp_fixed_cpp_path = 'n_Dp_fixed_inversion_' + data_inversion_duration[i] + '.txt'
...
for i in range(len(data_inversion_duration)):
    ...
    # print(f'mean_similarities of stack {i+1}: {mean_similarities}')
