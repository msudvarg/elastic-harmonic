...
void HK_Reader::read_csv2() {
    ...
    std::vector<double> hk_reading_time_user(2500, 0);
    std::vector<double> hk_reading_time_sys(2500, 0);
    ...
    while (true) {
        if (stop_thread_) {
            ...
            std::ofstream hk_reading_time_user_("../result_/run_time/hk_reading_time_user.txt");
            std::ofstream hk_reading_time_sys_("../result_/run_time/hk_reading_time_sys.txt");
            for(int i = 0; i < hk_reading_time.size(); ++i) {
                if(hk_reading_time_user[i] != 0 && hk_reading_time_user[i + 1] != 0) {
                   read_time_ << hk_reading_time[i] << " " << std::flush; 
                   hk_reading_time_user_ << hk_reading_time_user[i] << " " << std::flush;
                   hk_reading_time_sys_ << hk_reading_time_sys[i] << " " << std::flush;  
                }
            }
            read_time_.close();
            // read_time.close();
        ...
        hk_reading_time_user[loop] = user_time_millis;
        hk_reading_time_sys[loop] = system_time_millis;
