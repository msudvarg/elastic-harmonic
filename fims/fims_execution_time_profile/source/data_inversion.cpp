...
void Data_Inversion::perform_data_inversion() {
    ...
    // std::ofstream fixed_result_cpp("../../generate_result/result_/n_Dp_fixed_cpp.txt");
    ...
    std::vector<double> inversion_time_user(1250, 0);
    std::vector<double> inversion_time_sys(1250, 0);
    std::vector<double> inversion_time_(1250, 0);
    ...
    while (true) {
        // std::cerr << "current loop:" << ++loop << std::endl;
        // std::unique_lock<std::mutex> lock(data_mutex_);
        // data_cv_.wait(lock, [&] {
        ...
        if (stop_thread_) {
            std::ofstream inversion_time_user_("../result_/run_time/inversion_time_user.txt");
            std::ofstream inversion_time_sys_("../result_/run_time/inversion_time_sys.txt");
            std::ofstream inversion_time("../result_/run_time/inversion_time.txt");
            for(int i = 0; i < inversion_time_user.size() - 1; ++i){
                if(inversion_time_user[i] != 0 && inversion_time_user[i + 1] != 0) {
                    inversion_time_user_ << inversion_time_user[i] << " " << std::flush;
                    inversion_time_sys_ << inversion_time_sys[i] << " " << std::flush;
                    inversion_time << inversion_time_[i] << " " << std::flush;
                }
            }
            // loop_time.close();
            inversion_time_user_.close();
            inversion_time_sys_.close();
        ...
        if(cur_particles.empty()) {
            ...
            // result_cpp <<  "NaN" << std::endl;
            // fixed_result_cpp << ret << std::endl;
            // fixed_result_cpp << "NaN" << std::endl;
        ...
        inversion_time_user[loop] = user_time_millis;
        inversion_time_sys[loop] = system_time_millis;
