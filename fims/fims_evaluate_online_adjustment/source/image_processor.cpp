...
// Simulater, preload images to memory
void Image_Processor::fileread_n_process() {
    paramt_FIMS& fims = paramt_FIMS::getInstance();
    std::string dir = "../../raw_data/test_images";
    // std::string dir = "../../project_code/real_time_data/_00191#190904_003748";
    // std::string dir = "../../project_code/real_time_data/_00206#190904_005247";
    ...
    for (auto file : files) {
        ...
        std::string timestamp_ = filePath.stem().string();
        double timestamp = std::stod(timestamp_);
        ...
        if(init_time < 0) {
            ...
            // double t;
            // std::stringstream(timestamp) >> t;
            // t += i * cur_diff;
            timestamp = init_time + (timestamp - init_time) * fims.image_processing_duration / 100;
        }
        timestamp_ = std::to_string(timestamp);
        timestamps.push_back(timestamp_);
        images.push_back(img);
    }
    /******finish loading images to memory******/
    fims.finished_preloading = true;

    // std::ofstream process_cost("../result_/run_time/image_process_cost.txt");
    // std::ofstream image_process_cost_cpu("../result_/run_time/image_process_cost_cpu.txt");
    ...
    for(size_t i = 0; i < images.size(); ++i) {
        cv::Mat img = images[i];
        std::string timestamp = timestamps[i];
        ...
        // image_process_cost_cpu << elapsed << " ";
